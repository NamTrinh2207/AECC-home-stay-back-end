...
public interface IBookingRepository extends JpaRepository<Booking, Long> {
    @Query(nativeQuery = true, value = "SELECT h.id, h.address as address, h.name as name, h.price_by_day as pricebyday, ht.name as hometype, u.name as username,  COUNT( DISTINCT b.id) AS bookingcount,\n" +
            "       GROUP_CONCAT(hi.image) AS images\n" +
            "FROM homes h\n" +
            "         join booking b on h.id = b.home_id\n" +
            "         join home_type ht on h.home_type_id = ht.id\n" +
            "         join users u on h.user_id = u.id\n" +
            "         join homes_image hi on h.id = hi.homes_id\n" +
            "GROUP BY h.id, h.address, h.name, h.price_by_day, u.name\n" +
